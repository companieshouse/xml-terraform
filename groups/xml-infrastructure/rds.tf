...
module "xml_rds_security_group" {
  ...
  ingress_cidr_blocks = local.test_concourse_cidrs
  ingress_rules       = ["oracle-db-tcp"]
  ingress_with_cidr_blocks = var.test_concourse_rds_access_enable ? [
    {
      from_port   = 5500
      to_port     = 5500
      protocol    = "tcp"
      description = "Oracle Enterprise Manager"
      cidr_blocks = join(",", local.test_concourse_cidrs)
    }
  ] : []
