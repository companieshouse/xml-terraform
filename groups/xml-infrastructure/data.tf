...
data "vault_generic_secret" "xml_fe_data" {
...
data "aws_ami" "fe_xml" {
  ...
  most_recent = var.fe_ami_name == "xml-frontend-*" ? true : false
...
data "template_file" "fe_userdata" {
  ...
  vars = {
    ...
    LOG_GROUP_NAME      = aws_cloudwatch_log_group.xml_fe.name
...
# EWF Backend data
# ------------------------------------------------------------------------------
data "aws_ami" "bep_xml" {
  owners      = [data.vault_generic_secret.account_ids.data["development"]]
  most_recent = var.bep_ami_name == "xml-frontend-*" ? true : false
